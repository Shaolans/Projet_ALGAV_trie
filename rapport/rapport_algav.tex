...
\paragraph*{}
Nous souhaitions représenter un dictionnaire de mots, c'est-à-dire implanter une structure de
données efficace stockant des mots. Pour cela, nous allons nous servir de la notion de {\itshape trie}.
Dans cette optique, nous proposons les implantations de deux structures de tries différentes : le trie hybride
...
\begin{itemize}
 \item un caractère ``letter'', qui correspond à la lettre stockée.
 \item une valeur ``value'', qui correspond à la représentation de fin de mot. Si le nœud est la fin d'un mot, alors cette valeur est égale au nombre de mots déjà ajoutés + 1. Sinon elle est égale à -1.
 \item un pointeur vers un trie hybride ``fc'', ce trie hybride correspond au fils central. On le parcourt si le caractère recherché est le même que la lettre du trie hybride courant.
 \item un pointeur vers un trie hybride ``fg'', ce trie hybride correspond au fils gauche. On le parcourt si le caractère recherché est plus petit que la lettre du trie hybride courant selon l'ordre alphabétique.
 \item un pointeur vers un trie hybride ``fd'', ce trie hybride correspond au fils droit. On le parcourt si le caractère recherché est plus grand que la lettre du trie hybride courant selon l'ordre alphabétique.
\end{itemize}

\paragraph*{}
Voici une représentation du trie hybride résultant des ajouts successifs des mots suivants :
\begin{itemize}
 \item don
 \item le
 \item a
...
On peut considérer que les algorithmes suivant sont triviaux :
\begin{itemize}
...
\end{itemize}
En effet, ils correspondent essentiellement à un simple parcours de la structure du trie hybride, dont le traitement dépend de l'algorithme.
...
Voici une description succincte de l'algorithme :
\begin{itemize}
 \item si le trie hybride courant est un pointeur null alors on s'arrête.
 ...
 fils gauche ou droit suivant l'ordre alphabétique des caractères.
 \item on effectue un vérification de l'existence de mots dans les fils du trie hybride. Si un fils ne possède pas de nœud mot
...
Le comptage de pointeur vers NIL correspond à un parcours de tous les nœuds afin de compter le nombre de fils NIL.
Soit un arbre de taille \textit{n}, en prenant la comparaison de la valeur du nœud comme mesure,
on a une complexité en $\Theta$(n) puisque pour \textit{n} noeuds, on a un nombre de comparaisons $\le$ \textit{3n}.

\subsection{Hauteur}
La détermination de la hauteur du trie hybride est un parcours complet de l'arbre. Soit un trie hybride contenant n nœuds. On a alors une complexité en $\Theta$(n).

\subsection{Profondeur Moyenne}
La profondeur moyenne d'un trie hybride est un parcours jusqu'aux feuilles. On ajoute la profondeur de chaque branche dans une liste, puis on 
effectue la moyenne de ces profondeurs. Soit un trie hybride contenant n nœuds. On a alors  une complexité en $\Theta$(n).

\subsection{Préfixe}
La recherche du préfixe peut être dans le pire cas en $\Theta$(n). En effet, le pire cas est atteint lorsque le trie hybride
est une ``liste chaînée'' et donc ayant une hauteur \textit{h}=n.
...
dans le pire cas. Cependant, il y a aussi 3 appels une fonction qui recherche l'existence d'un nœud mot de complexité $\Theta$(n).
...
En effet, lors de la recherche, on est aiguillé par l'indice du PATRICIA trie, qui nous mène vers un de ses PATRICIA trie fils en $\Theta$(1). Si ce PATRICIA trie fils est une feuille, alors on compare sa valeur avec le mot à trouver, en $\mathcal{O}$(L) comparaisons de bits.
...
Nous avons calculé le temps de construction des différentes implantations. Nous avons chargé avec un pas de 1000,
...
D'après la courbe obtenue, le temps de construction d'un trie hybride et d'un PATRICIA trie semble être en $\Theta$(n), ce qui est en adéquation avec l'étude théorique.
La différence observée au niveau de la courbe est vraisemblablement due au fait que la valeur du PATRICIA trie est un préfixe, alors que celle du trie hybride est une lettre (ainsi il faut autant de nœuds que de lettres du préfixe).
...
Nous pouvons remarquer qu'ici, le temps d'ajout d'un mot est négligeable pour les deux structures  puisque la taille d'un mot français ne prend rarement de grande valeur (en nombre de lettres).

\section{Profondeur moyenne des structures}
Cette étude de la profondeur des structures permet de mettre en évidence l'évolution de leur profondeur moyenne suivant le nombre de mots contenus. Nous avons chargé les œuvres de Shakespeare avec un pas de 1000 et en calculant la profondeur
...
Au niveau de cette courbe, nous remarquons que la profondeur moyenne du PATRICIA trie est nettement inférieure au trie hybride, ce qui est en adéquation avec l'étude théorique. En effet, le PATRICIA trie a pour valeur le
préfixe des mots fils, alors que le trie hybride a pour valeur une lettre. Ainsi, il lui faut autant de nœuds que de lettres qu'il y a dans le préfixe.
...
Au niveau de cette courbe, nous remarquons que la hauteur du PATRICIA trie est nettement inférieure à celle du trie hybride, ce
qui est en adéquation avec l'étude théorique.
Pour un trie hybride, la hauteur de l'arbre peut être en grande partie due à la longueur du mot \textit{L} puisqu'un nœud
...
la longueur d'un mot peut au mieux être inclue entièrement dans un seul PATRICIA trie.
Nous pouvons aussi observer que le rééquilibrage permet dans certain cas de réduire la hauteur de l'arbre, cependant il n'est parfois pas possible de réduire en raison de la longueur du mot.
...
la plus efficace en terme de rapidité.
Cependant, le PATRICIA trie est bien plus gourmand en terme d'occupation mémoire. En effet, chaque nœud du PATRICIA trie a un tableau de 27 cases, afin de stocker ses fils, alors que pour le trie hybride n'a qu'au plus 3 fils.
Ainsi le PATRICIA trie serait plus adéquat si on a besoin d'un accès rapide aux mots, mais si on veut désire avoir une structure plus légère et que le temps de recherche n'est pas primordial, le trie hybride est plus adapté.
